...
module.exports = function(passport) {
  ...
  app.get('/topic-list', async function(req, res) {
  ...
  app.get('/getsubtopics', async function(req, res) {
    ...
    var query = 'select * from tbl_topic where parent_topic_id=? ORDER BY id ASC LIMIT 2';
    results = await database.query(query, [req.query.topic]);
    //console.log(results)
    // res.render('admin/topic/topic-list', {
    //     title: 'Topic List',
    //     data: JSON.parse(results)
  ...
  app.get('/getsubsubtopics', async function(req, res) {
    var query = 'select * from tbl_topic where parent_topic_id=? ORDER BY id ASC';
    results = await database.query(query, [req.query.subtopic]);
    //console.log(results)
    // res.render('admin/topic/topic-list', {
    //     title: 'Topic List',
    //     data: JSON.parse(results)
  ...
  app.get('/getTopics', async function(req, res) {
    var query = 'select * from tbl_topic where board_id=? and class_id=? and subject_id=? ORDER BY id ASC';
    var q = req.query;
    results = await database.query(query, [q.board, q.cls, q.subject]);
    console.log(results);
    // res.render('admin/topic/topic-list', {
    //     title: 'Topic List',
    //     data: JSON.parse(results)
  ...
  app.get('/show_subject', async function(req, res, next) {
    var query =
      'SELECT tbl_subjects.*, tbl_topic.name FROM tbl_subjects LEFT JOIN tbl_topic ON tbl_subjects.id = tbl_topic.subject_id WHERE tbl_topic.class_id = ' +
      req.query.id +
      '  ';
  ...
  app.post('/addtopic', async function(req, res, next) {
    req.assert('name', 'Topic Name is required').notEmpty();
